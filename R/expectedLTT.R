expectedLTT <- function(pars, dt = 0.1, ct=15, n_it= 100, value = 0, grid){ # it should also include model
  #grid = seq(0,ct, by=dt)
  if(value>0){
    grid = value
  }
  ...
  for (i in 1:(n_it)){
    s = sim_phyl(ct = ct, lambda0 = pars[1], mu0 = pars[2], K = pars[3],seed=(2000+i))
